...
APP_STORE_CONNECT_WEBHOOK_SECRET_NAME="${APP_STORE_CONNECT_WEBHOOK_SECRET_NAME:-}"
...
if [ -n "${APP_STORE_CONNECT_WEBHOOK_SECRET_NAME}" ]; then
  require_existing_secret "${APP_STORE_CONNECT_WEBHOOK_SECRET_NAME}" \
...
print(json.dumps({
    ...
    "TESTFLIGHT_FEEDBACK_EMAIL": os.environ.get("TESTFLIGHT_FEEDBACK_EMAIL", ""),
    "TESTFLIGHT_APP_NAME": os.environ.get("TESTFLIGHT_APP_NAME", "RID2Caltopo"),
    "TESTFLIGHT_APP_APPLE_ID": os.environ.get("TESTFLIGHT_APP_APPLE_ID", "6792518823"),
...
if [ -n "${APP_STORE_CONNECT_WEBHOOK_SECRET_NAME}" ]; then
  SECRET_MAPPINGS="${SECRET_MAPPINGS},APP_STORE_CONNECT_WEBHOOK_SECRET=${APP_STORE_CONNECT_WEBHOOK_SECRET_NAME}:latest"
fi
if [ -n "${CLOUDFLARE_TURN_KEY_ID_SECRET_NAME}" ]; then

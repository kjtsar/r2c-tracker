...
STRIPE_WEBHOOK_SECRET_SECRET_NAME="${STRIPE_WEBHOOK_SECRET_SECRET_NAME:-}"
...
export STRIPE_WEBHOOK_SECRET_SECRET_NAME
...
if [ -n "${STRIPE_SECRET_KEY_SECRET_NAME}" ] \
  || [ -n "${STRIPE_WEBHOOK_SECRET_SECRET_NAME}" ]; then
  if [ -z "${STRIPE_SECRET_KEY_SECRET_NAME}" ] \
    || [ -z "${STRIPE_WEBHOOK_SECRET_SECRET_NAME}" ]; then
    echo "Both Stripe Secret Manager names must be configured together." >&2
    exit 1
  fi
fi
...
if [ -n "${STRIPE_SECRET_KEY_SECRET_NAME}" ]; then
  require_existing_secret "${STRIPE_SECRET_KEY_SECRET_NAME}" "STRIPE_SECRET_KEY"
  require_existing_secret "${STRIPE_WEBHOOK_SECRET_SECRET_NAME}" "STRIPE_WEBHOOK_SECRET"
fi
...
if [ -n "${STRIPE_SECRET_KEY_SECRET_NAME}" ]; then
  SECRET_MAPPINGS="${SECRET_MAPPINGS},STRIPE_SECRET_KEY=${STRIPE_SECRET_KEY_SECRET_NAME}:latest"
